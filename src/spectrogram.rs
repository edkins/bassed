...
pub fn get_spectrogram(info: &ProjectAudio, spec: &ProjectSpectrogram, index: usize) -> Option<Vec<u8>> {
    ...
    let start_pos = info.channels * 4 * num_samples * index;
    ...
    let array = Array::from_vec(f32_vec).into_shape((info.channels, num_samples)).ok()?;
    ...
    for i in 0..info.channels {
        for j in 0..num_steps {
            ...
            input_view.assign(&array.slice(s![i, offset..offset+spec.samples_per_fft]));
